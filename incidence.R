...
df <-
  read_excel(here::here("data", "incidence.xlsx"), sheet = "Total")
...
df %<>%
  mutate(
    Type = ifelse(Type == "SIDS deaths", "SIDS", "SUDI"),
    Rate = Number * 1000 / `Live births`
  ) %>%
  rowwise %>%
  mutate(
    Rate_lower_CI = ifelse(
      Type == "SUDI",
      poisson.test(Number)$conf.int[1] * 1000 / `Live births`,
      NA
    ),
    Rate_upper_CI = ifelse(
      Type == "SUDI",
      poisson.test(Number)$conf.int[2] * 1000 / `Live births`,
      NA
    )
  )
...
incidence_plot <- ggplot(data = df) +
  ...
  geom_errorbar(
    aes(x = Year, ymin = Rate_lower_CI, ymax = Rate_upper_CI),
    size = .5,
    width = 0.2,
    color = "gray50"
  ) +
...
rate_df_p <-
  ggtexttable(select(rate_df,-c("SUDI lower CI", "SUDI upper CI")),
              rows = NULL,
              theme = ttheme("default"))
