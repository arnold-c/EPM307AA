...
df <- read_excel(here::here("data", "incidence.xlsx"), sheet = "Ethnic Group")
...
df %<>%
  pivot_longer(cols = c(`2006`:`2015`), 
               names_to = "Year", values_to = "Number") %>%
...
SIDS_inc <- ggplot(data = df, 
  ...
  geom_line(aes(group = `Ethnic Group`), 
            size = 2) +
  geom_point(size = 4) +
  scale_color_manual(values = alpha(c("#0073C2FF", "#EFC000FF", "#CD534CFF",
                                      "#868686FF", "black"), 
                                    c(0.3, 0.3, 0.3, 0.3, 1))) +
